...
cap = cv2.VideoCapture('circle_detection_input.mp4')
ret, frame = cap.read()
H, W, _= frame.shape
#out = cv2.VideoWriter(video_path_out, cv2.VideoWriter_fourcc(*'MJPG'), int(cap.get(cv2.CAP_PROP_FPS)), (W, H))
out = cv2.VideoWriter('circle_detection_out_4.avi', cv2.VideoWriter_fourcc(*'MJPG'), int(cap.get(cv2.CAP_PROP_FPS)), (W, H))
